...
export function Popup({ isOpen = false, onClosePopup }: Props) {
  ...
  return (
    <div
      ...
      className={cn(
        "fixed top-0 left-0 w-screen h-screen bg-[#041c2ae6] z-50 transition-opacity duration-500 flex justify-center items-center",
        popupClassname
      )}
    >
      <div
        ref={popupRef}
        className="bg-tertiary shadow-sm w-full justify-center p-7 pt-6 md:p-4 md:pt-4 text-white md:max-w-[600px] md:py-8 md:px-10 border border-secondary/20"
      >
        ...
        <div className="flex flex-col justify-center items-center text-center gap-3 md:gap-6 border-t border-white/50">
          <span className="font-GothamBook text-xxs pt-3 md:text-tiny md:pt-6 tracking-widest">
            FINAL RELEASE OF PENTHOUSES & TOWNHOMES
          </span>
          <p className="font-americana-bold text-lg md:text-2xl">
            Tour Our New Display Homes <br /> By Appointment Only
          </p>
          <div className="grid grid-cols-2 gap-2 md:gap-3">
            {popupImages.map((item, index) => (
              ...
              >
                <Image
                  src={item.image}
                  alt={`${item.collection} interior image`}
                  className="size-[90%] md:size-full"
                />
                <div className="flex flex-col">
                  <p className="uppercase tracking-widest text-tiny md:text-xs">
          ...
          <Link
            href="https://calendly.com/tailorbymarcon"
            target="_blank"
            type="button"
            className="flex items-center uppercase bg-secondary/20 text-white text-tiny mt-3 md:mt-1 px-12 py-4 tracking-widest hover:bg-secondary/50"
          >
            Book an appointment
          </Link>
        </div>
