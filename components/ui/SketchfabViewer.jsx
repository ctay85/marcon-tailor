...
export default function SketchfabViewer({ setActiveHome, activeLevel }) {
  ...
  const onAnnotationSelect = (i) => {
    ...
    viewerInstances[activeModel].api.getAnnotation(i, (err, annotation) => {
      ...
      const formattedPlanType =
        unitNumber.substring(0, 2) === "26" ? `S${planType}` : planType;
  ...
  const initModel = (key, instance) => {
    ...
    instance.api.addEventListener("viewerready", () => {
      ...
      instance.api.getAnnotationList((err, annotations) =>
        setViewerInstances({
          ...viewerInstances,
          [key]: {
            ...viewerInstances[key],
  ...
  useEffect(() => {
    if (activeModel) {
      ...
      viewerInstances[activeModel].api.addEventListener(
        "annotationSelect",
        onAnnotationSelect
      );
    }
    ...
    return () => {
      if (activeModel) {
        ...
        viewerInstances[activeModel].api.removeEventListener(
          "annotationSelect",
          onAnnotationSelect
        );
      }
  ...
  useEffect(() => {
    ...
    if (activeModel) {
      console.log(
        "activeLevel or allModelsLoaded changed, updating annotations"
      );
      updateAnnotations(activeModel);
    }
  ...
  //
  useEffect(() => {
    const isTypicalFullyLoaded =
      viewerInstances.typical.api && viewerInstances.typical.annotations;
    const isPenthouseFullyLoaded =
      viewerInstances.penthouse.api && viewerInstances.penthouse.annotations;
    const isTH1FullyLoaded =
      viewerInstances.townhome_level_1.annotations &&
      viewerInstances.townhome_level_1.annotations;
    const isTH2FullyLoaded =
      viewerInstances.townhome_level_2.annotations &&
      viewerInstances.townhome_level_2.annotations;
    const isTH3FullyLoaded =
      viewerInstances.townhome_level_3.annotations &&
      viewerInstances.townhome_level_3.annotations;
    ...
    if (
      isTypicalFullyLoaded &&
      viewerInstances.penthouse.api &&
      !viewerInstances.penthouse.annotations
    ) {
    ...
    if (
      isPenthouseFullyLoaded &&
      viewerInstances.townhome_level_1.api &&
      !viewerInstances.townhome_level_1.annotations
    ) {
    ...
    if (
      isTH1FullyLoaded &&
      viewerInstances.townhome_level_2.api &&
      !viewerInstances.townhome_level_2.annotations
    ) {
    ...
    if (
      isTH2FullyLoaded &&
      viewerInstances.townhome_level_3.api &&
      !viewerInstances.townhome_level_3.annotations
    ) {
  ...
  return (
    <div className="sketchfab-embed-wrapper">
      <iframe
        className="model--typical"
        data-active={activeModel === "typical"}
        ref={iframeModelTypical}
      ></iframe>
      <iframe
        className="model--penthouse"
        data-active={activeModel === "penthouse"}
        ref={iframeModelPenthouse}
      ></iframe>
      <iframe
        className="model--th-lvl-1"
        data-active={activeModel === "townhome_level_1"}
        ref={iframeModelTH1}
      ></iframe>
      <iframe
        className="model--th-lvl-2"
        data-active={activeModel === "townhome_level_2"}
        ref={iframeModelTH2}
      ></iframe>
      <iframe
        className="model--th-lvl-3"
        data-active={activeModel === "townhome_level_3"}
        ref={iframeModelTH3}
      ></iframe>
    </div>
  );
