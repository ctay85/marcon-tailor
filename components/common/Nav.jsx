...
const nav = [
  ...
  { label : 'Design', cls : 'page__index__design' },
...
const childVariants = {
  hidden : { y : '100%', transition : { duration : .4, ease : [0, .5, 0, 1] } },
  visible : { y : 0, transition : { duration : .4, ease : [0, .5, 0, 1] }}
}
...
export default function Nav ({ setActivePanelClass, activePanelClass }) {
  ...
  //
  return (
    <motion.section className="global__nav" animate={ navOpen ? 'visible' : 'hidden' } initial="hidden" variants={ parentVariants }>
      <nav>
        {
          nav.map(({ label, cls }, i) => (
            <button
